...
class Game {
  ...
  makeHtmlBoard() {
    ...
    // Creates a bind for this to be used for the handleClick
  ...
  handleClick(evt) {
    ...
    if (this.checkForWin()) {
      ...
      if (winner.color === this.players[0].color) {
...
function creategameForm() {
  const gameForm = document.getElementById('gameSettings');
  ...
  gameForm.addEventListener('submit', function (evt) {
    ...
    // Create players here with submitted colors. then send the colors to the Game object
    new Game(p1, p2, 6, 7); // player 1, player 2, height, width
    // new Player()
  });
}
